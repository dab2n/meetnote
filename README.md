...
`docs/`는 정적 뷰어다. 빌드가 없고 `index.html` 하나로 돈다. 회의록을 미리 만들어 두지 않는다 —
**기록 파일을 넣으면 그 자리에서 논의 구조를 뽑아 시간순으로 펼쳐 보여준다.**

- **넣는 것** — 전사 `.txt`(`이름 00:00` 줄 + 발언 줄), 또는 `.transcript.json`. 음성(`.m4a`)을 같이 넣으면 재생 위치가 연결된다
- **처리** — 전부 브라우저 안에서 끝난다. 어디로도 업로드하지 않는다
- **보이는 것** — 검정 카드가 **주제**, 그 오른쪽으로 **의견**이 갈라진다(제안=파랑 실선 · 동의=초록 실선 · 반대=빨강 파선 · 질문=회색 점선).
  갈라진 의견은 다시 하나로 모이고, 결론이 난 주제는 **주황 카드**, 안 난 주제는 회색 점선 **미결** 카드로 닫힌다
- **전개** — 파일을 넣으면 한 번에 다 뜨지 않고 발언 시각 순서대로 하나씩 등장한다. 상단에서 일시정지 · 속도(1·2·4×) · 다시. 끝나면 완료 메시지가 뜬다
- 모든 발언을 그리지 않는다. 길이 · 결론/반대/제안 어휘 · 주제 전환을 점수로 매겨 주제당 2–4개만 남긴다
- 노드나 원문의 어느 지점을 눌러도 **그 시각의 음성으로 이동**한다
