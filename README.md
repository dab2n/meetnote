...
## 1-2) 화면 우상단에 고정해두기 (macOS) — 이게 메인이다
...
- 배경을 끌면 옮길 수 있고, 다시 켜면 우상단으로 돌아온다
...
| 권한 | 없으면 | 주는 곳 |
|---|---|---|
| 마이크 | 내 목소리가 안 들어감 | 첫 실행 시 프롬프트 |
| 화면 기록 | 상대방 목소리가 안 들어감 (macOS는 시스템 오디오를 여기 묶어놨다) | 시스템 설정 → 개인정보 보호 및 보안 → 화면 기록 → `meetnote` 켜기 |

패널의 **권한 열기 →** 를 누르면 그 설정 창이 바로 열린다. 켠 뒤 `./pin.sh`로 다시 띄운다.
권한이 없어도 녹음은 되고, 뭐가 빠졌는지 패널에 표시된다.
