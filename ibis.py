...
## 3. issue — 그 논의의 쟁점 문장을 기준에 맞게 다시 쓴다
- 두 갈래에서 **고르는** 자리였으면 물음으로: `~인가` · `~할 것인가` (25~45자)
- 값·대상·범위를 **좁혀 정하는** 자리, 또는 각자 해 온 것을 **공유·점검**한 자리였으면
  명사형 안건 이름으로 (8~24자, 서술이나 물음으로 끝내지 않는다)
지금 문장이 이미 맞으면 그대로 둔다. 없던 대립을 만들지 않는다.

## 4. flow — 논의마다 '오간 이야기'를 문장으로 (2~4문장)
...
def audit(m: dict, segs: list[dict] | None = None, partial: bool = False) -> list[str]:
    ...
    for sec in secs_:
        ...
        for n in nodes + [dict(sec.get("resolution", {}), id=f"{sec.get('no')}-res")]:
            ...
            if k == "issue" and not re.search(r"(인가|것인가|는가|은가|운가|을까|ㄹ까|까)[?]?$", t):
                lo, hi = 8, 30                                   # 명사형 쟁점은 짧다 (SPEC 2절)
            if not lo <= len(t) <= hi:
