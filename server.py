...
def process(audio: Path, target: str):
    try:
        print(f"[{audio.parent.name}] 전사 중...", flush=True)
        text = meetnote.transcribe(audio)
        (audio.parent / "transcript.txt").write_text(text)
        print(f"[{audio.parent.name}] 요약 중...", flush=True)
        s = meetnote.summarize(text)
        dest = meetnote.export(s, audio.parent, target)
        print(f"[{audio.parent.name}] 완료 -> {dest}", flush=True)
        subprocess.run(["open", dest])
    except Exception:
        traceback.print_exc()
        print(f"[{audio.parent.name}] 실패. out/ 안의 중간 결과를 확인하세요.", flush=True)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        audio = d / f"audio.{ext}"
        audio.write_bytes(self.rfile.read(size))
        print(f"수신: {audio} ({size/1e6:.1f}MB) -> {target}", flush=True)

        # 전사+요약은 몇 분 걸린다. 익스텐션을 붙잡아두지 않고 백그라운드로 넘긴다.
        threading.Thread(target=process, args=(audio, target), daemon=True).start()
