...
```bash
python ibis.py 전사문.txt --id 2026-05-30 --date 2026.05.30 --local
python ibis.py 전사문.txt --id 2026-05-30 --local --attach 기획서.pdf 화면.png   # 회의자료도 함께 읽는다
```

`./pin.sh` 로 서버가 떠 있으면 뷰어에서도 된다 — `http://127.0.0.1:8787/notes/` 로 열면
**데스크탑에서 만들기 · 추가 비용 없음** 버튼이 먼저 나온다. **＋ 회의자료** 로 pdf·이미지·문서를
붙이면 CLI 가 그 파일을 직접 읽고 회의 내용과 대조한다.

공개 링크(GitHub Pages)에서는 이 경로를 쓸 수 없다. 내 맥의 서버에 붙어야 하기 때문이다.

### 정리 맵 만들기 — 웹에서

**내 맥의 주소(`http://127.0.0.1:8787/notes/`)로 열면 전사문을 놓는 순간 바로 만들기 시작한다.**
이 맥의 Claude Code 로 돌아 구독 안에서 처리되고 추가 비용이 없다. 버튼을 누를 필요도 없다.

공개 링크(GitHub Pages)에서는 만들 수 없다 — https 페이지가 내 맥의 http 서버에 닿지 못한다.
공개 링크는 **만들어진 회의를 보고 나누는** 용도다. 굳이 거기서 만들려면 자기 API 키를 넣어야 하고,
그건 구독이 아니라 **키 계정으로 따로 청구**된다.

1. (공개 링크에서만) 오른쪽 위 **⚙︎** 에 자기 Anthropic API 키를 넣는다
2. 전사문 `.txt` 를 놓는다
3. **정리 맵 만들기** — 응답이 오는 대로 **논의가 하나씩 캔버스에 쌓인다.**
   스트림에서 완성된 섹션을 집어내 그때마다 다시 그린다 (미리 만든 것을 꺼내는 게 아니다).
   초안 → 원문 대조 → 표준 검사 → 수리 순으로 돌고, 단계마다 경과 시간이 찍힌다.
   첫 응답까지 1~3분 조용할 수 있다 (모델이 전사문을 다 읽고 생각하는 구간이다)
   - **앞 15분만 시험** — 같은 3단계를 앞부분만으로 돌린다. 처음 붙일 때 이걸로 확인하면 비용이 몇 분의 일이다
   - **실행할 때마다 넣어 둔 키의 계정으로 과금된다.** 구독(claude.ai·Claude Code)과는 별개다.
     83분 회의 전체가 대략 **$1~$3** (사고 토큰이 출력에 함께 과금되고, 수리 단계까지 가면 위쪽).
     끝나면 **실제로 쓴 토큰과 금액**이 화면과 콘솔에 찍힌다 — 추정이 아니라 응답에서 받은 값이다.
     console.anthropic.com 에서 지출 한도를 걸어 두는 편이 안전하다
4. 만들어진 맵은 이 브라우저에 남아 첫 화면 목록에 뜬다. **내려받아** `docs/data/` 에 넣고
   `index.json` 에 한 줄 추가하면 링크로 공유된다 (내려받은 파일은 전사문까지 품고 있어 그 하나면 된다)

키는 브라우저에만 저장되고 Anthropic 외 다른 곳으로 가지 않는다.
정리를 위해 **전사문이 Anthropic으로 전송된다** — 회의 내용이 밖으로 나가면 곤란하면 아래 CLI를 쓴다.
...
전사문은 **시각이 붙은 화자 줄**이 있어야 한다 (음성·노드·원문을 이으려면 시각이 필요하다).
`시헌 00:00` · `발화자 1  (00:00)` · `참석자 3 [1:02:03]` 를 모두 읽고,
클로바노트처럼 **UTF-16**으로 떨어진 파일도 BOM을 보고 알아서 디코딩한다.
...
### 표준과 검사 — 어떤 회의를 넣어도 같은 수준이 나오게

규칙은 **[`docs/SPEC.md`](docs/SPEC.md) 한 곳에만** 적는다. `ibis.py`가 그 파일을 그대로 읽어 프롬프트에 넣고,
같은 규칙을 `audit()`이 기계로 검사한다. 규칙을 바꾸려면 `docs/SPEC.md`만 고친다. 웹 뷰어도 이 파일을 그대로 읽어 쓴다.

생성은 세 단계로 돌고, 검사를 통과할 때까지 되돌아온다.

```
초안  →  원문과 대조  →  표준 검사  →  어긴 항목만 수리 (최대 2회)
                              ↑____________________|
```

`audit()`이 보는 것 — 섹션 수, 섹션당 노드 수, 쟁점 하나 규칙, 주장 유무,
**종류별 종결 형태**(주장 `~한다`, 우려 `~가 걸린다`, 미결 `~는 정하지 못했다` …),
제목 길이, **발언 인용 여부**(전사문에 그대로 있으면 위반), parent가 같은 섹션인지,
대립 3조건 중 기계로 볼 수 있는 둘(말한 사람이 다른가 · 주장끼리인가),
확정 비율(3분의 2 초과면 오탐 의심), 하이라이트 형식(`~한 순간`, 이름 미포함, quote).

사람이 판단해야 하는 것(정말 팽팽했는가, 정말 합의였는가)은 검사하지 않고 프롬프트와 대조 패스가 맡는다.

**표본은 NEWTON 회의**(`docs/data/2026-06-15.ibis.json`)다. 새 맵은 그 수준을 목표로 하고,
표본 자신이 표준을 통과하는지는 테스트가 지킨다(`python test_meetnote.py`).

```bash
python ibis.py --audit docs/data/2026-06-15.ibis.json
#  2026-06-15.ibis.json · 논의 14개 · 위반 0건
```
